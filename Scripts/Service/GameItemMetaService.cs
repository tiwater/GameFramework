...
namespace GameFramework.GameStructure.Service
{
    public class GameItemMetaService
    {
        ...
        /// <param name="handler"></param>
        /// <returns></returns>
        public IEnumerator GetGameItemMetas(GameItemMetasHandler handler)
        {
            ...
            yield return (HttpUtil.GetDummy(GlobalConstants.SERVER_TISVC_PREFIX + "/{AppId}/GameItemMeta", webRequest =>
            {
                AddressableGameItemMeta[] itemMetas = MockItems();
                if (handler != null)
                {
